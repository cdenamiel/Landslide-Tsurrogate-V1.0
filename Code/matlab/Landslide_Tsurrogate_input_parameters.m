function Landslide_Tsurrogate_input_parameters
...
disp(' ')
disp('----------------------------------------------------------------------------')
disp('STEP 2: generation of the input parameters for the deterministic simulations')
disp('----------------------------------------------------------------------------')
disp(' ')
disp(' ')
disp('!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
...
users='../results/output_users.mat';
if exist(users, 'file')
  load(users);
else
  % File does not exist.
...
param = surrogate_model_gauss_patterson_psa_param(maxdeg,nmodes,a,b,option);
sim_atlas=param(maxdeg+1).Z;
nnn=length(sim_atlas);
disp(' ')
disp(['The total number of deterministic simulations to run is: ',num2str(nnn)])
disp(' ')

%------------------------------------------------------
% Match the indices of the simulations at maximum order 
% with lower orders
%------------------------------------------------------

for i = 1:maxdeg+1
    sim = param(i).Z;
    [~,~,ind]=intersect(sim,sim_atlas,'rows');
    param(i).index = ind;
end
...
save ../results/output_param.mat param
disp('The results have been saved in: ../results/output_param.mat')
disp(' ')

end

function param = surrogate_model_gauss_patterson_psa_param(maxdeg,nmodes,a,b,option)
%% Pseudo Spectral Approximation with Gauss-Patterson Sparse grids
%--------------------------------------------------------------------------
% This routine is used prior to the model runs. 
% It assumes that all the stochastic variables are uniformly distributed. 
% 
% The routine defines the parameters to be used in the model runs and store
% them in the matrix Z.The corresponding weights W.
% The Smolyak (1963) algorithm is used. 
%
% based on:
...
%-------
%
% maxdeg: maximum value of the norm 1 of the multi-indeces of the
%         multivariate distribution = maximum degree of the PCE of the
%         solution in any direction
%         dimension: (1)
%
% nmodes: number of stochastic variables (directions)
%         dimension: (1)
%
...
%--------------------------------------------------------------------------
% 
% Start computation of parameters for pseudospectral method using GP quadrature rule
%
param = struct([]);   
for nmaxdeg = 0:maxdeg
    disp(['maxdeg = ' num2str(nmaxdeg)])
    Z      = []; 
    for alpha_norm1 = max(0,nmaxdeg-nmodes+1):nmaxdeg              
        ...
        for l=1:size(alpha,1)
            ...
            [Znew,~] = gauss_patterson_nested_rule_nd(alpha(l,:),a,b,option);            
            Z = [Z;Znew]; %#ok<*AGROW>
        end
    end
    % Sort the nodes and merge repeated values
    Z = unique(single(Z),'rows');           
    % Final sorting of the nodes
    [Z,~] = sortrows(Z);    
    % store the final values in strcture param
    param(nmaxdeg+1).Z     = Z;
end
return
end
