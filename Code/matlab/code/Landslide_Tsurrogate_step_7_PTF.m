function Landslide_Tsurrogate_step_7_PTF
...
disp('STEP 4: Run Landslide_Tsurrogate_step_4_format_input to generate the file: ../results/output_model.mat')
disp('STEP 5: Run Landslide_Tsurrogate_step_5_coefficients to generate the file: ../results/output_coeff.mat')
disp('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
disp(' ')
...
%--------------------------------------------------------------------------------------
% In this section the user defines the limits of the distributions to generate the PTF 
%--------------------------------------------------------------------------------------
...
% chosen number of samples for the PTF
nw0 = 1000;
...
if (length(a0) ~= nmodes) || (length(b0) ~= nmodes)
    disp('The defined input for PTF')
...
tic
PTF = surrogate_model_gauss_patterson_PTF(maxdeg0,a,b,coeff,nw0,a0,b0);
disp(' ')
disp(' ---------------------------------------------------------------')
disp(['PTF based on ',num2str(nw0),' members:'])
toc
disp(' ---------------------------------------------------------------')

save ../results/output_PTF.mat PTF
disp(' ')
disp('The results have been saved in: ../results/output_PTF.mat')
disp(' ')

end

function PTF = surrogate_model_gauss_patterson_PTF(maxdeg,ar,br,coeff,nw,a,b)
...
for alpha_norm1 = max(0,maxdeg-nmodes+1):maxdeg
    ...
    PTF.zeta=exp(zeta_temp);
    PTF.velo=exp(velo_temp);
    PTF.time=exp(time_temp);
end
end