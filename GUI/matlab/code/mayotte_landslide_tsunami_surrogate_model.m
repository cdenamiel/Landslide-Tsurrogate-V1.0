...
% surrogate models, display the results and save the output in surrogate_results.mat.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Initialisation
%--------------------------------------------------------------------------
...
elseif or(min_Friction < 4 , max_Friction > 12)
    ...
    %
    output.input_surrogate.zone = zone_name;
    if (s(1) == 0)
    	output.input_surrogate.distance.value  = [a(1) b(1)];
    else
    	output.input_surrogate.distance.value  = a(1);
    end
    output.input_surrogate.distance.units = 'm';
    output.input_surrogate.distance.long_name = 'Distance from the southern point of the landslide zone.';
    if (s(2) == 0)
    	output.input_surrogate.volume.value  = [a(2) b(2)];
    else
    	output.input_surrogate.volume.value  = a(2);
    end
    output.input_surrogate.volume.units = 'Mm3';
    output.input_surrogate.volume.long_name = 'Volume of the landslide';
    if (s(3) == 0)
    	output.input_surrogate.friction.value  = [a(3) b(3)];
    else
    	output.input_surrogate.friction.value  = a(3);
    end
    output.input_surrogate.friction.units = 'degree';
    output.input_surrogate.friction.long_name = 'Angle of friction of the landslide.';
    output.max_ele.value          = max_ele;
    output.max_vel.value          = max_vel;
    output.max_tide_ele.value     = max_tide_ele;
    output.max_tide_vel.value     = max_tide_vel;
    output.max_ele.units = 'm';
    output.max_ele.long_name = 'Maximum Tsunami Elevation including tide';
    output.max_tide_ele.units = 'm';
    output.max_tide_ele.long_name = 'Maximum Tidal Elevation extracted with TMD TPXO 7.2 model';
    output.max_vel.units = 'm/s';
    output.max_vel.long_name = 'Maximum Tsunami Currents including tide';
    output.max_tide_vel.units = 'm/s';
    output.max_tide_vel.long_name = 'Maximum Tidal Currents extracted with TMD TPXO 7.2 model';
    save ../results/surrogate_results.mat output
    %    
